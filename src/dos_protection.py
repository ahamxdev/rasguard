...
def block_ip(ip):
    ...
    if ip not in blocked_ips:
        ...
        subprocess.call(['sudo', 'iptables', '-I', 'FORWARD', '-s', ip, '-j', 'DROP'])
        blocked_ips.add(ip)
...
def process_packet(packet):
    ...
    if scapy_pkt.haslayer(IP) and scapy_pkt.haslayer(TCP):
        if server_ip != scapy_pkt[IP].src:
            ...
            if ip_src not in blocked_ips and (
                ip_requests[ip_src]["SYN"] > SYN_LIMIT or
                ip_requests[ip_src]["ACK"] > ACK_LIMIT or
                ip_requests[ip_src]["FIN"] > FIN_LIMIT or
                ip_requests[ip_src]["CONN"] > CONN_LIMIT):

                message = f"Possible DoS Attack from {ip_src}."
                print(message)
                send_sms(message)
